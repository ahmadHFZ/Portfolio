...
export default function ClientHome({
  ...
  const getLocalized = (item: any, field: string) => {
    ...
    if (
      locale === "de" &&
      item[`${field}_de`] &&
      item[`${field}_de`].length > 0
    )
      return item[`${field}_de`];
    if (
      locale === "ar" &&
      item[`${field}_ar`] &&
      item[`${field}_ar`].length > 0
    )
  ...
  return (
    <Box
      className="bg-black text-white min-h-screen font-sans selection:bg-purple-500 selection:text-white"
      dir={dir}
    >
      <ScrollObserver />
          ...
          >
            ...
            >
              ...
              >
                <Typography
                  variant="h2"
                  className="font-bold mb-2 bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent leading-tight"
                >
                ...
                <Typography
                  variant="h4"
                  className="text-gray-200 mb-6 font-light"
                >
                  {getTitle()}
                </Typography>
                ...
                <div
                  className={`flex ${dir === "rtl" ? "flex-row-reverse" : ""} gap-4 flex-wrap`}
                >
                  {profile?.cvUrl && (
                    <Button
                      variant="contained"
                  ...
                  <div
                    className={`flex ${dir === "rtl" ? "flex-row-reverse" : ""} gap-2`}
                  >
                    {profile?.socialLinks?.github && (
                      <IconButton
                        ...
                        className="hover:bg-gray-100 transition-colors border-2 border-white"
                      >
                        <FaGithub size={28} />
                      </IconButton>
                    )}
                    {profile?.socialLinks?.linkedin && (
                      <IconButton
                        ...
                        className="hover:bg-blue-700 transition-colors border-2 border-blue-600"
                      >
                        <FaLinkedin size={28} />
                      </IconButton>
                    )}
                    {profile?.socialLinks?.twitter && (
                      <IconButton
                        ...
                        target="_blank"
                        className="bg-[#1DA1F2] text-white hover:bg-[#0c85d0] transition-colors border-2 border-[#1DA1F2]"
                      >
                        <FaTwitter size={28} />
                      </IconButton>
                    )}
                    {profile?.socialLinks?.instagram && (
                      <IconButton
                        ...
                        target="_blank"
                        className="bg-gradient-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] text-white hover:opacity-90 transition-colors border-2 border-pink-500"
                      >
                        <FaInstagram size={28} />
                      </IconButton>
                    )}
      ...
      {skills.length > 0 && (
        <section id="skills" className="py-20 bg-gray-900/50">
          <Container>
            <Typography
              variant="h3"
              className="font-bold mb-12 text-center bg-gradient-to-r from-green-400 to-blue-500 bg-clip-text text-transparent"
            >
                ...
                <Grid size="auto" key={skill._id}>
                  ...
                  >
                    ...
                    <Typography className="font-medium text-gray-200">
                      {getLocalized(skill, "name") || skill.name}
                    </Typography>
                  </motion.div>
                </Grid>
              ))}
      ...
      {projects.length > 0 && (
        <section className="py-20 bg-black">
          <Container>
            <Typography
              variant="h3"
              className="font-bold mb-16 text-center bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent"
            >
            ...
            >
              {projects.map((project) => (
                <SwiperSlide
                  key={project._id}
                  className="w-[300px] md:w-[400px] bg-transparent"
                >
      ...
      {education.length > 0 && (
        <section className="py-20 bg-gradient-to-b from-black to-gray-900">
          <Container maxWidth="lg">
            <Typography
              variant="h3"
              className="font-bold mb-12 text-center bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent"
            >
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-2">
                      <Typography variant="h5" className="font-bold text-white">
                        {getLocalized(edu, "degree") || edu.degree}
                      </Typography>
                      <Chip
                        label={edu.year}
                        className="bg-yellow-500 text-black font-bold mt-2 md:mt-0 w-fit"
                      />
                    </div>
                    <Typography
                      variant="h6"
                    ...
                    >
                      ...
                      {getLocalized(edu, "institution") || edu.institution}
                    </Typography>
      ...
      {certifications && certifications.length > 0 && (
        <section className="py-20 bg-black">
          <Container maxWidth="lg">
            <Typography
              variant="h3"
              className="font-bold mb-12 text-center bg-gradient-to-r from-blue-400 to-cyan-500 bg-clip-text text-transparent"
            >
      ...
      {books.length > 0 && (
        <section className="py-20 bg-gray-900">
          <Container maxWidth="lg">
            <Typography
              variant="h3"
              className="font-bold mb-12 text-center bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent"
            >
              ...
              {books.map((book, index) => (
                <Grid size={{ xs: 12, sm: 6, md: 4 }} key={book._id}>
                  ...
                  >
                    ...
                    <Typography
                      variant="h6"
                      className="font-bold text-center mb-2 group-hover:text-cyan-400 transition-colors"
                    >
      ...
      {photos.length > 0 && (
        <section className="py-20 bg-black">
          <Container>
            <Typography
              variant="h3"
              className="font-bold mb-12 text-center text-white"
            >
              {t.gallery || "Gallery"}
            </Typography>
      ...
      <section className="py-20 bg-gradient-to-t from-gray-900 to-black">
        <Container maxWidth="md" className="text-center">
          <Typography
            variant="h3"
            className="font-bold mb-8 bg-gradient-to-r from-gray-200 to-gray-400 bg-clip-text text-transparent"
          >
